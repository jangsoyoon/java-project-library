...
function notice_list_content(notice){
	return `<tr id="table2">
		<td width=5% align=center class=t2><font size=2 color=#000000>${notice.notice_no}</td>
		<td width="300" bgcolor="ffffff" style="padding-left: 10">
		
		
				<a href="#" class="notice_list" notice_no="${notice.notice_no}" 'style="color:#1c7b3a; font-weight:900;"':'style="color:black;"'>
				
				${
								function(){
									var html='';
									
									if(notice.board_status=='공지사항'){
										html=`<img src="img/notice_d.png" width=150px; height=40px;>`;
									}
									
									for(var i=0; i<notice.board_depth;i++){
										html+=`&nbsp;&nbsp;&nbsp&nbsp&nbsp`;
									}
									if(notice.board_depth>0){
		...
		<td width=7% align=right class=t3><font size=2 color=#000000>${notice.notice_no}</font></td>
		
		<td width=10% align=center class=t3><font size=2 color=#000000>${notice.notice_date}</font></td>
		<td width=5% align=center class=t3><font size=2 color=#000000>${notice.notice_readcount}</font></td>
		</tr>`;
}
/*
...
function notice_list_content(noticeArray,pageArray) {
	return `<table>
		...
		<tbody>
			<!--requestBoard start-->
				${
					noticeArray.map(notice_item_content).join('')
				}
			<!--requestBoard end-->
		</tbody>
